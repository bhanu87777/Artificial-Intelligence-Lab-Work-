...
def bfs(initial_state):
    ...
    # A queue for BFS (stores state, parent, and move to reach the state)
    queue = deque([(initial_state, None)])  # The queue stores tuples (state, parent)
    ...
    while queue:
        ...
        for move_x, move_y in moves:
            ...
            if 0 <= new_x < 3 and 0 <= new_y < 3:  # Make sure the move is within bounds
                ...
                # If the new state has not been visited, add it to the queue and update the parent map
